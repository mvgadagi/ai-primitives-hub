...
suite('ScopeSelectionUI - Property Tests', () => {
  ...
  const setWorkspaceOpen = (isOpen: boolean): void => {
    (globalThis as any).__mockWorkspaceFolders = isOpen
  ...
  setup(() => {
    ...
    createMockQuickPick();
    mockCreateQuickPick = sandbox.stub().callsFake(() => mockQuickPick);
    (globalThis as any).__mockCreateQuickPick = () => mockCreateQuickPick();
  ...
  teardown(() => {
    sandbox.restore();
    delete (globalThis as any).__mockCreateQuickPick;
    // Restore original workspace folders
    (globalThis as any).__mockWorkspaceFolders = originalWorkspaceFolders;
  });
